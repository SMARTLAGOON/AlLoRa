...
def handle_command(command, type):
	...
	if type == "request-data-info":
		...
		if log_file.first_sent is None:
...
def t_file(file, t0_tf):
	...
	if t0_tf:
		file.first_sent = t
		txt = "{} kb: t0 -> {}".format(file.get_name(), t)
		test_log.write(txt)
		print(txt)
	else:
		if file.first_sent is not None:
			file.last_sent = t
			txt = "{} kb: tf -> {}: SST = {}; Retransmition = {} []\n".format(file.get_name(), t, t - file.first_sent, file.retransmition)
			test_log.write(txt)
			print(txt)
	test_log.close()
	return t
